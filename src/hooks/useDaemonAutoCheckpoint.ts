...
export function useDaemonAutoCheckpoint(wsId: string | null) {
  ...
  const run = useCallback(async (reason: string, force = false) => {
    ...
    try { await daemonService.syncCheckpoint(id, reason); }
  ...
  useEffect(() => {
    if (!wsId) return;
    const leaving = wsId;
    return () => { void daemonService.syncCheckpoint(leaving, 'handoff').catch(() => { /* noop */ }); };
  }, [wsId]);

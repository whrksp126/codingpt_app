...
export default function CloudWorkspaceSheet({ visible, onClose, onCreated }: {
  ...
  const designate = useCallback(async () => {
    ...
    if (!finalPath) { alert({ title: i18n.t('이름 필요'), message: i18n.t('워크스페이스 폴더 이름을 입력하거나 폴더를 먼저 만들어 주세요.') }); return; }
    const displayName = nm || finalPath.slice(finalPath.lastIndexOf('/') + 1) || i18n.t('워크스페이스');
    setBusy(true);
    try {
      ...
      alert({ title: i18n.t('생성 실패'), message: e?.message || String(e) });
      setBusy(false);
    }

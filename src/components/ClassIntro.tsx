...
// 다크모드용 색 토큰
const useShowcaseColors = () => {
  const { resolvedScheme } = useTheme();
  const isDark = resolvedScheme === 'dark';
  return {
    isDark,
    surface: isDark ? '#1B1F27' : '#ffffff',
    surfaceAlt: isDark ? '#23272F' : '#f9fbf7',
    border: isDark ? '#3F444D' : '#eef1e9',
    borderSoft: isDark ? '#3F444D' : '#e7efd9',
    borderAccent: isDark ? '#2F5A1A' : '#dff0cf',
    text: isDark ? '#FFFFFF' : '#111315',
    textMuted: isDark ? '#9CA3AF' : '#5d646f',
    accentBg: isDark ? '#1F3018' : '#eaffd5',
    accentBgSoft: isDark ? '#1A2A12' : '#eef7e8',
    accentText: isDark ? '#A6E22E' : '#2b6a00',
    halo1: isDark ? '#1A2A12' : '#eaffd5',
    halo2: isDark ? '#1B1F27' : '#ffffff',
    cardEdge: isDark ? '#2F5A1A' : '#c8f0a0',
    pillBg: isDark ? '#1A2A12' : '#eefae6',
    pillBorder: isDark ? '#2F5A1A' : '#cfeec2',
    badgeChipBg: isDark ? '#1A2A12' : '#f3f9ef',
    badgeChipBorder: isDark ? '#2F5A1A' : '#e7f3de',
    statCardBg: isDark ? '#1B1F27' : '#ffffff',
  };
};
    ...
    <View style={{ backgroundColor: c.surface }}>
      {blocks.map((b, i) => {
        switch (b.type) {
          ...
          case 'divider': return <View key={i} className="my-2 h-[1px]" style={{ backgroundColor: c.border }} />;
          default: return null;
        }
      })}
    </View>
...
const Headline = ({ kicker, title, subtitle, hero, tags, stats }: any) => {
  ...
  return (
    <View className="rounded-3xl overflow-hidden mb-6" style={{ shadowColor:'#000', shadowOpacity:0.08, shadowRadius:12, elevation:3 }}>
      <View className="relative" style={{ backgroundColor: c.surface }}>
        <View className="absolute -top-10 -right-12 w-44 h-44 rounded-full" style={{ backgroundColor: c.halo1 }} />
        <View className="absolute -bottom-14 -left-16 w-52 h-52 rounded-full" style={{ backgroundColor: c.halo2 }} />
        <View className="px-4 pt-5 pb-4">
          {kicker && (
            <View className="self-start flex-row items-center gap-2 px-3 py-1.5 rounded-full" style={{ backgroundColor: c.badgeChipBg, borderColor: c.badgeChipBorder, borderWidth: 1 }}>
              <View className="w-2 h-2 rounded-full" style={{ backgroundColor:'#58CC02' }} />
              <Text className="text-[12px] font-semibold" style={{ color: c.accentText }}>{kicker}</Text>
            </View>
          )}
          <Text className="mt-2 text-[22px] font-extrabold" style={{ color: c.text }}>{title}</Text>
          {subtitle ? <Text className="mt-1 text-[15px]" style={{ color: c.textMuted }}>{subtitle}</Text> : null}
          ...
          {tags?.length ? (
            <ScrollView horizontal showsHorizontalScrollIndicator={false} className="-mx-4 mt-3 pl-4" contentContainerStyle={{ paddingRight:16 }}>
              {tags.map((tag: string, idx: number) => (
                <View key={idx} className="mr-2 rounded-full px-3 py-1.5" style={{ backgroundColor: c.accentBgSoft, borderColor: c.borderAccent, borderWidth: 1 }}>
                  <Text className="text-[13px] font-bold" style={{ color: c.accentText }}>#{tag}</Text>
                </View>
              ))}
            </ScrollView>
          ) : null}
          ...
          {stats?.length ? (
            <View className="mt-3 flex-row gap-2">
              {stats.map((s: any, idx: number) => (
                <View key={idx} className="flex-1 items-center rounded-xl px-3 py-2" style={{ backgroundColor: c.statCardBg, borderColor: c.border, borderWidth: 1 }}>
                  <Text className="text-[13px]" style={{ color: c.textMuted }}>{s.label}</Text>
                  <Text className="pt-0.5 text-[15px] font-bold" style={{ color: c.text }}>{s.value}</Text>
                </View>
              ))}
            </View>
          ) : null}
    ...
    <View className="mb-6">
      {items.map((it, k) => (
        <View key={k} className="mb-3 rounded-2xl overflow-hidden" style={{ shadowColor:'#000', shadowOpacity:0.1, shadowRadius:14, elevation:3 }}>
          {/* 엣지 그라디언트 밴드 */}
          <View style={{ height: 6, backgroundColor: c.cardEdge }} />
          <View className="relative p-5" style={{ backgroundColor: c.surface, borderColor: c.borderSoft, borderWidth: 1 }}>
            {/* 입체 배지 - 절대 위치 */}
            <View className="absolute -top-[-5] -right-0 rounded-full px-4 py-1.5 mr-1.5" style={{ backgroundColor: c.pillBg, borderColor: c.pillBorder, borderWidth: 1 }}>
              <Text className="text-[12px] font-bold" style={{ color: c.accentText }}>{it.emoji} HIGHLIGHT</Text>
            </View>
            {/* 타이틀 + 설명 */}
            <View className="flex-row items-center py-2">
              <View className="mr-2 rounded-2xl items-center justify-center" style={{ width: 45, height: 45, backgroundColor: c.accentBg }}>
                <Text className="text-[25px]">{it.emoji}</Text>
              </View>
              <View className="flex-1">
                <Text className="text-[17px] font-extrabold" style={{ color: c.text }}>{it.title}</Text>
                <Text className="mt-0.5 text-[14px]" style={{ color: c.textMuted }}>{it.desc}</Text>
              </View>
            </View>
          </View>
        </View>
...
const GameMosaic = ({ headline = '게임처럼, 쉽게!', sub = '퀘스트를 깨며 코딩이 익숙해져요', badges, image }: any) => {
  ...
  return (
    <View className="mb-6 rounded-2xl overflow-hidden" style={{ shadowColor:'#000', shadowOpacity:0.07, shadowRadius:10, elevation:2 }}>
      ...
      <View className="px-3 py-3" style={{ backgroundColor: c.surface, borderTopWidth: 1, borderTopColor: c.border }}>
        <Text className="text-[22px] font-extrabold" style={{ color: c.text }}>{headline}</Text>
        <Text className="text-[15px] mt-0.5" style={{ color: c.textMuted }}>{sub}</Text>
        <View className="mt-3 flex-row flex-wrap -mx-1">
          {badges.slice(0, 6).map((b: any, i: number) => (
            <View key={i} className="w-1/2 px-1 mb-2">
              <View className="rounded-xl px-2 py-2" style={{ backgroundColor: c.surfaceAlt, borderColor: c.border, borderWidth: 1 }}>
                <Text className="text-[20px]">{b.emoji}</Text>
                <Text className="text-[17px] font-semibold mt-0.5" style={{ color: c.text }}>{b.title}</Text>
                <Text className="text-[13px]" style={{ color: c.textMuted }}>{b.desc}</Text>
              </View>
            </View>
          ))}
        </View>
...
const FancyTimeline = ({ title, subtitle, items }: { title?: string; subtitle?: string; items: { step: string; title: string; desc: string }[] }) => {
  const c = useShowcaseColors();
  return (
    <View className="py-3 mb-6 rounded-2xl overflow-hidden" style={{ backgroundColor: c.surface, shadowColor:'#000', shadowOpacity:0.08, shadowRadius:12, elevation:2 }}>
      <View className="px-3 pt-3">
        {title ? <Text className="text-[22px] font-extrabold" style={{ color: c.text }}>{title}</Text> : null}
        {subtitle ? <Text className="text-[13px] mt-0.5" style={{ color: c.textMuted }}>{subtitle}</Text> : null}
      </View>
      <View className="px-3 pb-3">
        {items.map((it, k) => (
          <View key={k} className="flex-row mt-3">
            ...
            {/* 스텝 카드 */}
            <View className="flex-1 rounded-xl px-3 py-2" style={{ backgroundColor: c.surface, borderColor: c.border, borderWidth: 1, shadowColor:'#000', shadowOpacity:0.06, shadowRadius:8, elevation:2 }}>
              <Text className="text-[15px] font-semibold" style={{ color: c.text }}>{it.title}</Text>
              <Text className="text-[13px]" style={{ color: c.textMuted }}>{it.desc}</Text>
            </View>
          </View>
        ))}
      </View>

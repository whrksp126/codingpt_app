...
export default function ComputeStatusButton() {
  ...
  const { localOnline, hasDevice, cloudOnline } = useDaemonStatus();
  const { setActiveWorkspace } = useAgentSession();
  const { openIde } = useIdeProject();

  const enterPcTerminal = useCallback(() => {
    const pid = daemonProjectId(''); // 데몬 홈 루트(터미널은 트리 로드와 독립적으로 즉시 뜸)
    setActiveWorkspace({ id: pid, name: '내 PC', kind: 'project' });
    navigation.navigate('Tabs', { screen: 'home' });
    openIde({ ide: { projectId: pid, projectName: '내 PC', openTerminal: true } });
  }, [navigation, setActiveWorkspace, openIde]);

  const onPress = useCallback(() => {
    if (localOnline) enterPcTerminal();
  ...
  return (
    ...
    >
      <StatChip icon={<Desktop size={14} color={C.text2} weight="fill" />} dot={pcDot} />
      <View style={{ width: 1, height: 12, backgroundColor: C.border }} />
      <StatChip icon={<Cloud size={14} color={C.text2} weight="fill" />} dot={cloudDot} />
    </Pressable>
  );
}

function StatChip({ icon, dot }: { icon: React.ReactNode; dot: string }) {
  return (

...
export const listWorkspaces = () =>
  unwrap(apiRequest<{ workspaces: WorkspaceMeta[] }>('/api/workspaces', { method: 'GET' }), i18n.t('워크스페이스를 불러올 수 없습니다.'));

/** 워크스페이스 생성 — 폴더(메타) 생성 */
export const createWorkspace = (input: CreateWorkspaceInput = {}) =>
  unwrap(apiRequest<{ workspace: WorkspaceMeta }>('/api/workspaces', { method: 'POST', body: input }), i18n.t('워크스페이스를 만들 수 없습니다.'));

/** 단일 워크스페이스 메타 */
export const getWorkspace = (workspaceId: string) =>
  unwrap(apiRequest<{ workspace: WorkspaceMeta }>(`/api/workspaces/${workspaceId}`, { method: 'GET' }), i18n.t('워크스페이스를 찾을 수 없습니다.'));

/** 이름 변경 등 메타 패치 */
export const updateWorkspace = (workspaceId: string, patch: Partial<CreateWorkspaceInput> & { unread?: number }) =>
  unwrap(apiRequest<{ workspace: WorkspaceMeta }>(`/api/workspaces/${workspaceId}`, { method: 'PATCH', body: patch }), i18n.t('워크스페이스를 수정할 수 없습니다.'));

/** 워크스페이스 복제 */
export const duplicateWorkspace = (workspaceId: string) =>
  unwrap(apiRequest<{ workspace: WorkspaceMeta }>(`/api/workspaces/${workspaceId}/duplicate`, { method: 'POST' }), i18n.t('복제에 실패했습니다.'));

/** 워크스페이스 삭제 */
export const deleteWorkspace = (workspaceId: string) =>
  unwrap(apiRequest<{ id: string; deleted: number }>(`/api/workspaces/${workspaceId}`, { method: 'DELETE' }), i18n.t('삭제에 실패했습니다.'));

/** 프로젝트 그룹에서 분리 — 새 단독 프로젝트로(자동 연결 오판 교정, 영구 저장) */
export const detachProject = (workspaceId: string) =>
  unwrap(apiRequest<{ workspace: WorkspaceMeta }>(`/api/workspaces/${workspaceId}/project/detach`, { method: 'POST' }), i18n.t('프로젝트 분리에 실패했습니다.'));

/** 다른 워크스페이스의 프로젝트에 합치기 */
export const attachProject = (workspaceId: string, targetWorkspaceId: string) =>
  unwrap(apiRequest<{ workspace: WorkspaceMeta }>(`/api/workspaces/${workspaceId}/project/attach`, { method: 'POST', body: { targetWorkspaceId } }), i18n.t('프로젝트 합치기에 실패했습니다.'));

/** 설명 → 워크스페이스 이름 후보 추천(신규 생성 플로우) */
export const suggestWorkspaceNames = (description: string): Promise<string[]> =>
  unwrap(apiRequest<{ names: string[] }>('/api/workspaces/suggest-name', { method: 'POST', body: { description } }), i18n.t('이름 추천에 실패했습니다.')).then((d) => d.names);

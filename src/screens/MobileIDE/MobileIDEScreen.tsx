...
export default function MobileIDEScreen({ ide, lessonId, visible = true, onClose }: MobileIDEProps) {
  ...
  // (구) 바이브코딩 에이전트 결합 제거 — AI 채팅은 터미널의 claude 로 일원화됐다.
  //  활성 워크스페이스 포인터는 IdeProjectContext 가 보유(setActiveWorkspace).
  const { activeWorkspace, setActiveWorkspace } = useIdeProject();

  // 데몬(내 PC) IDE 를 닫을 때: pc: 워크스페이스가 활성으로 남지 않게 포인터를 비운다.
  const handleClose = useCallback(() => {
    if (isDaemon) { try { setActiveWorkspace(null); } catch (_) { /* noop */ } }
    onClose?.();
  }, [isDaemon, setActiveWorkspace, onClose]);
  // 실행 중 여부 — dev 서버 상태 표시에서 참조.

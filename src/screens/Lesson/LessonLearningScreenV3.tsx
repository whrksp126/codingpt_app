...
const LessonLearningScreenV3: React.FC<Props> = ({ route, navigation }) => {
  // =========================
  // 📌 기본 설정
  // =========================
  ...
  return (
    <>
      ...
      >
        {/* 세그먼트형 프로그레스 바 */}
        <View className="flex-row gap-[4px] px-[16px] pt-[16px]">
          {Array.from({ length: curLesson?.sliders.length || 0 }).map((_, index) => (
            <View 
              key={`progress-segment-${index}`}
              className={`flex-1 h-[3px] rounded-[10px] ${
                index < visibleSlides.length ? 'bg-Success-700_Default' : 'bg-Line-White'
              }`}
            />
          ))}
        </View>
        ...
        {/* 소제목과 X 버튼 */}
        <View className="flex-row items-center justify-between px-[16px] h-[50px]">
          {/* 소제목 */}
          <Text className="text-Text-Black_Secondary text-16_bold">
            {curLesson?.sliders[curSlideIndex]?.role || curLesson?.sliders[curSlideIndex]?.title || ""}
          </Text>
          ...
          <DefaultIconBtn
            ...
            bounceScale={1.15}
          >
            <X width={24} height={24} fill="rgba(51, 51, 51, 0.8)" />
          </DefaultIconBtn>
        </View>
      </View>
...
export default LessonLearningScreenV3;
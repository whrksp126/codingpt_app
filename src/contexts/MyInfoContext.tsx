...
type Step = 'info' | 'settings' | 'account';
...
type MyInfoContextValue = {
  ...
  pushAccount: () => void;    // info → account(계정)
  back: () => void;           // settings/account → info
  close: () => void;          // 시트 닫기
...
export const MyInfoProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const back = useCallback(() => setStep('info'), []);
  ...
  return (
    <MyInfoContext.Provider value={{ open, step, githubOpen, openSheet, pushSettings, pushAccount, back, close, openGithub, closeGithub }}>
      {children}

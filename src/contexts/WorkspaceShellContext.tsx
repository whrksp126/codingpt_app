...
function reconcilePool(rt: WsRuntime, wins: { index: number; name: string; command?: string; agent?: string | boolean | null; agentName?: string | null; agentReady?: boolean | null; agentState?: string | null }[]): WsRuntime {
  ...
  const rec = (node: TilingNode): TilingNode | null => {
    if (T.isLeaf(node)) {
      ...
      node.tabs.forEach((t, i) => {
        ...
        const agentReady = typeof w.agentReady === 'boolean' ? w.agentReady : undefined;
        ...
        if (t.miss || (w.name && t.title !== w.name) || (t.cmd || '') !== cmd
            ...
            || (t.agentReady ?? undefined) !== agentReady
            || (t.agentState ?? undefined) !== agentState) {
          changed = true;
          tabs.push({ ...t, miss: undefined, title: w.name || t.title, cmd, agent, agentName, agentReady, agentState });
          return;
        }
        tabs.push(t);
      });
  ...
  if (missing.length) {
    ...
    const tabsToAdd: T.TerminalTab[] = missing.map((w) => ({
      ...
      agentReady: typeof w.agentReady === 'boolean' ? w.agentReady : undefined,
            ...
            for (const [id, w] of Object.entries<any>(saved.ws)) {
              if (w && w.layout) {
                ...
                T.eachLeaf(layout, (l) => {
                  ...
                  for (const t of l.tabs) { if ('agent' in t) delete t.agent; if ('agentName' in t) delete t.agentName; if ('agentReady' in t) delete t.agentReady; if ('agentState' in t) delete t.agentState; }

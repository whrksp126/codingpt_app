...
// ★ TUI ↔ Chat 토글은 **여기(main-top)에 없다.** 2026-07-27 오전에 사용자 요구("메인 영역 기준 우측
//  상단")를 앱 헤더로 잘못 읽어 전역 1개로 옮겼다가 같은 날 되돌렸다 — 원래 의도는 **터미널 pane 본문의
//  우측 상단**이고, 렌더 정본은 `PaneView`(pane 마다 자기 것) + `chat/ModeToggle`(절대배치 오버레이)다.
//  헤더로 올리면 pane 이 둘 이상일 때 "어느 터미널의 모드인가"가 화면에서 사라지고, 그 결함을 메우려고
//  대상 폴백·포커스 이동 규칙을 계속 덧붙여야 했다. 여기에 다시 만들지 말 것(uiControls 의 모드 채널도
//  같은 이유로 삭제했다 — pane 이 자기 setTabMode 를 직접 호출한다).
...
export default function WorkspaceView() {
  ...
  // 지금 활성 pane 에서 보고 있는 터미널 id — 팔레트가 "이 터미널에서" 하는 동작에 쓴다.
  //  터미널 pane 이 아니거나 아직 win 이 안 잡힌 'new' 탭이면 null 이다.

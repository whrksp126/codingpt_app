...
//
// PC(codingpt_pc/src/js/preview-kind.js)의 **1:1 이식**이다. 한쪽에서만 png 를 그림으로 열면
//  같은 파일이 기기마다 다르게 보인다 — 대조 테스트가 두 파일을 묶어 둔다.
//
// 규율:
...
export function needsBytes(kind: PreviewKind): boolean {
  return kind === 'image' || kind === 'pdf' || kind === 'audio' || kind === 'video';
}

/** 기본이 미리보기인가(= 열자마자 미리보기). svg 는 그림으로 열되 토글이 있다. */
export function opensAsPreview(kind: PreviewKind): boolean {
  return kind !== 'text';
}

/** 미리보기에서 텍스트로 되돌릴 수 있는가. 바이너리는 되돌려도 깨진 글자뿐이라 막는다. */
export function canFallBackToText(kind: PreviewKind): boolean {
  return kind === 'markdown' || kind === 'svg' || kind === 'table' || kind === 'json';
}

/** data: URI — 이미지·PDF·미디어를 화면에 물릴 때 쓴다. */
export function dataUri(name: string, base64: string): string {
 ...
 */
export function parseTable(text: string, ext = 'csv', maxRows = 500): { rows: string[][]; truncated: boolean } {
  const sep = ext === 'tsv' ? '\t' : ',';
  const rows: string[][] = [];
  let field = '';
  let row: string[] = [];

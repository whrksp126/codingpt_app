...
function humanVideoNote(raw?: string): string {
  ...
  //  "이 기기는 라이브 화면을 지원하지 않아요" 도 같은 경우다 — 에이전트 PC 를 모르는 구 데몬(0.1.346 이전)이 기기 탓을
  //   한다. 기기 문제가 아니라 PC 앱 버전 문제라고 말한다(2026-09-20 폰 실기에서 사용자가 혼란).
  if (/지원하지 않는 스트림|라이브 화면을 지원하지/.test(m)) {

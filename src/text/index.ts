...
//
// 2026-08-05 다국어(7종)를 켜면서 **번역 계층은 `src/i18n/` 하나로 통일**했다. 여기 사전들은
//  이제 "한국어 원문 모음"일 뿐이고, 실제 언어 선택은 `tx()` 가 `i18n.t()` 로 넘긴다.
//  (예전엔 이 파일이 `{ko, en}` 두 벌을 직접 골랐다 — 두 메커니즘이 공존하면 새 화면은 이쪽,
//   옛 화면은 저쪽이 되어 "어디를 고쳐야 하는지"가 사람마다 달라진다. 옛 `en` 값들은 지우기 전에
//   전부 `i18n/en.json` 으로 회수했다.)
//
// 규율:
//  · 값에 문장 조립(`'파일 ' + n + '개'`)을 넣지 않는다 → 함수 값이거나 `t('파일 {n}개', {n})`.
//  · 새 문구는 여기 넣든 화면에서 `t('…')` 로 바로 쓰든 상관없다. 어느 쪽이든 같은 사전을 탄다.
//
// ⚠ PC(`codingpt_pc/src/js/text/index.js`)에 같은 구조가 있다.

/** 사전의 모양 — 이제 한국어 한 벌이다(번역은 i18n 카탈로그가 갖는다). */
export type Dict<T> = { ko: T };
 ...
 */
function lazy<T>(obj: T): T {
  if (!obj || typeof obj !== 'object') return obj;
  const hit = cache.get(obj as unknown as object);
  if (hit) return hit as T;
  const proxy = new Proxy(obj as unknown as Record<string, unknown>, {
    get(target, prop) {
      const v = Reflect.get(target, prop);
      if (typeof v === 'string') return i18n.t(v);
      if (v && typeof v === 'object') return lazy(v);
      // 함수 값은 그대로 — 안에서 t() 를 부르는 건 그 함수의 몫이다.
      return v;
    },
  });
  cache.set(obj as unknown as object, proxy);
  return proxy as T;
}

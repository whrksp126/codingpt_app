...
p('앱 ↔ PC 대칭 핀(소스 수준)', () => {
  ...
  it('앱 토글은 WorkspaceView 헤더가 아니라 PaneView(그 pane)에서 렌더된다', () => {
    ...
    expect(/resolveToggleVisible\(\{ isTerm: activeIsTerm, win: activeWin, chatMode, agentOn, chatReady, betaOn: chatBetaOn \}\)/.test(pv)).toBe(true);
    ...
    expect(/ApprovalBanner|TuiApprovalDock/.test(pv)).toBe(false);
    expect(/QuestionDock/.test(fs.readFileSync(path.resolve(__dirname, '../src/workspace/chat/ChatBody.tsx'), 'utf8'))).toBe(true);
